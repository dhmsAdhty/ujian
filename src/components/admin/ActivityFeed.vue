...
const getColorClass = (type) => {
  ...
  return 'bg-venus-100 text-venus-600'
}
</script>
...
<template>
  <GlassCard>
    <div class="mb-5 flex items-center justify-between gap-3">
      <h3 class="text-lg font-semibold tracking-tight text-venus-900">Aktivitas terkini</h3>
      <span
    ...
    <div v-if="loading" class="space-y-3">
      <div v-for="i in 5" :key="i" class="h-14 animate-pulse rounded-lg bg-venus-100" />
    </div>

    <div v-else-if="activities.length === 0" class="py-8 text-center text-sm text-venus-500">
      Belum ada aktivitas tercatat.
    </div>

    <div v-else class="relative space-y-5">
      <div class="absolute top-2 bottom-2 left-[19px] w-px bg-venus-100" aria-hidden="true" />
      ...
      >
        ...
        <div class="min-w-0 flex-1 pt-0.5">
          <div class="flex items-start justify-between gap-2">
            <h4 class="truncate text-sm font-medium text-venus-900">{{ activity.title }}</h4>
            <time
              class="shrink-0 text-[11px] tabular-nums text-venus-400"
              :datetime="activity.time"
            >
              {{ formatDistanceToNow(new Date(activity.time), { addSuffix: true }) }}
            </time>
          </div>
          <p class="mt-0.5 line-clamp-2 text-sm text-venus-500">{{ activity.description }}</p>
        </div>
      </div>
    </div>
  </GlassCard>
