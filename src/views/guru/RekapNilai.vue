    ...
    html: `
      <div class="text-left space-y-4">
        <div class="p-3 bg-slate-50 rounded-lg border border-slate-100">
          <p class="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Pertanyaan Essay</p>
          <p class="text-sm font-bold text-slate-700">Jelaskan cara kerja fotosintesis pada tumbuhan hijau?</p>
        </div>
        ...
        <div class="p-4 bg-white border border-primary-100 rounded-xl shadow-sm">
          ...
          <p class="text-sm text-slate-600 leading-relaxed font-bold">Proses tumbuhan buat makanan pake matahari dan klorofil terus ngeluarin oksigen juga.</p>
        </div>
        <div class="mt-4">
          <label class="block text-xs font-bold text-slate-500 mb-1">Berikan Nilai (0-100)</label>
          <input id="swal-input-score" class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:border-primary-500 outline-none" type="number" value="85">
        </div>
      </div>
    `,
...
<template>
  <div class="space-y-8 animate-fade-in">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
      <div>
        <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Rekap Nilai Siswa</h1>
        <p class="text-slate-500 mt-1">Pantau perolehan skor dan lakukan koreksi manual jawaban essay.</p>
      </div>
      ...
      <div class="flex flex-col lg:flex-row gap-4 items-center">
        <div class="w-full lg:flex-1">
          <div class="relative group">
            <Search class="absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-400" :size="20" />
            <input type="text" placeholder="Cari nama siswa..." class="form-input pl-11" />
          </div>
        </div>
        <div class="flex gap-4 w-full lg:w-auto">
          <select v-model="selectedExam" class="form-input lg:w-64 bg-white border-slate-200">
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse">
          ...
          <tbody v-else class="divide-y divide-slate-50">
            <tr v-for="res in results" :key="res.id" class="group hover:bg-slate-50/50 transition-all duration-300">
              <td class="py-5 px-6">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <h4 class="font-bold text-slate-800 tracking-tight">{{ res.profiles?.full_name }}</h4>
                    <span class="text-[10px] text-slate-400 uppercase font-black tracking-widest">NIS: 1240102{{ res.id.split('-')[0].charAt(0) }}</span>
                  </div>
                </div>
              </td>
              <td class="py-5 px-6">
              ...
              <td class="py-5 px-6 text-center">
                <span class="text-lg font-black text-slate-800 tracking-tight">{{ res.pgScore }}</span>
              </td>
              <td class="py-5 px-6 text-center">
                <span 
              ...
              <td class="py-5 px-6 text-right">
                <div class="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button 
                    ...
                    class="p-2 bg-white border border-slate-100 rounded-xl text-primary-500 hover:shadow-md transition-all flex items-center gap-1.5"
                  >
                    <Edit3 :size="18" />
                    <span class="text-xs font-bold">Koreksi</span>
                  </button>
                  <button class="p-2 bg-white border border-slate-100 rounded-xl text-slate-400 hover:text-slate-600 hover:shadow-md transition-all">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      ...
      <div class="glass-card p-6 flex items-center justify-center border-dashed">
         <div class="text-center">
           <p class="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Rata-rata Kelas</p>
           <h3 class="text-4xl font-black text-slate-800 tracking-tighter">78.5</h3>
         </div>
      </div>
    </div>
  </div>
